...
function criarBotaoNao(comAnimacao) {
    ...
    // Usa a tradução atual para o texto do botão
...
function ativarFuga() {
    ...
    // Remove o botão inicial
    if (btnNaoInicial) {
        btnNaoInicial.remove();
    }
    
    // Cria o botão fugitivo
    criarBotaoNao(true);
}
    ...
    ativarFuga();
            ...
            function getIdiomaDaURL() {
                try {
                    ...
                    if (langParam) {
                        // Mapeia os parâmetros possíveis para os códigos do sistema
                        const paramMap = {
...
window.addEventListener('load', function() {
    ...
    // Prioridade: 1º Parâmetro da URL, 2º localStorage, 3º Idioma do navegador
